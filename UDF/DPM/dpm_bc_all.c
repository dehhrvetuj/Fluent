...
For the return status PATH_ACTIVE, the particle continues to track (reflect). 
For the return status PATH_ABORT, the particle will be stopped and considered to be aborted (abort). 
For the return status PATH_STOP, the particle will be stopped and considered to be incomplete (incomplete). 
For the return status PATH_END, the particle will be stopped as well, but considered to have escaped from the domain (escape).
...
/* or alternatively in the following way  */
DEFINE_DPM_BC(bc_reflect, p, t, f, f_normal, dim)
{
  real alpha;  /* angle of particle path with face normal */
  real vn=0.;
